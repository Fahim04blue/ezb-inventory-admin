    ...
    return (
      <Card>
        <CardContent className="pt-8 text-sm text-muted-foreground">
          No rate records match the current filters.
        </CardContent>
      </Card>
    );
  }
      ...
      <div className="hidden overflow-hidden rounded-3xl border border-border bg-card shadow-sm md:block">
        <div className="overflow-x-auto">
          <div className="min-w-[1080px]">
            <div className="grid grid-cols-[minmax(0,0.9fr)_minmax(0,1fr)_minmax(0,0.8fr)_minmax(0,0.8fr)_minmax(0,0.8fr)_minmax(0,0.7fr)_minmax(0,1fr)_minmax(0,1fr)] gap-4 border-b border-border px-4 py-3 text-sm font-semibold text-muted-foreground">
            ...
            <div className="divide-y divide-border">
              {rates.map((rate) => (
                <div
                  className="grid grid-cols-[minmax(0,0.9fr)_minmax(0,1fr)_minmax(0,0.8fr)_minmax(0,0.8fr)_minmax(0,0.8fr)_minmax(0,0.7fr)_minmax(0,1fr)_minmax(0,1fr)] items-center gap-4 px-4 py-3 hover:bg-muted/50"
                  key={rate.id}
                >
                  <div className="text-sm text-muted-foreground">{formatDate(rate.effectiveDate)}</div>
                  <div>
                    <p className="font-medium">{rate.displayType}</p>
                    <p className="mt-1 text-xs text-muted-foreground">{rate.rateName}</p>
                  </div>
                  ...
                  <div className="text-sm text-muted-foreground">{rate.note || "No notes"}</div>
                  <div className="flex flex-wrap gap-2">
                    ...
                    <Button className="h-8 px-3 text-xs" onClick={() => onToggleStatus(rate)} variant="outline">
                      {rate.displayStatus === "DISABLED" ? "Activate" : "Disable"}
                    </Button>
                  </div>
                </div>
              ))}

    ...
    <div className="space-y-4 md:hidden">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">Create and manage normal orders and pre-orders.</p>
        </div>
        <Button className="h-10 w-auto shrink-0 rounded-xl bg-emerald-800 px-3.5 text-white" onClick={props.onAddOrder}>
          ...
          {props.isLoading ? (
            <div className="space-y-3">
              {Array.from({ length: 3 }).map((_, index) => (
                <div className="h-44 animate-pulse rounded-3xl bg-white" key={index} />
              ))}
            </div>
          ) : orders.length ? (
            <div className="space-y-3">
            ...
            <div className="rounded-3xl border border-slate-200/80 bg-white px-5 py-8 text-center shadow-sm">
              <h2 className="font-semibold text-slate-950">
                {activeTab === "ACTIVE" ? "No active orders found" : activeTab === "PRE_ORDERS" ? "No pre-orders found" : "No completed orders found"}
              </h2>
              <p className="mt-1 text-xs text-slate-500">Create an order or clear filters.</p>
            </div>
          )}
          ...
          {props.totalPages > 1 ? (
            <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white p-2.5 text-xs text-slate-500">
              ...
              <div className="flex gap-1.5">
                <Button aria-label="Previous page" className="h-8 w-8 px-0" disabled={props.currentPage <= 1} onClick={() => props.onPageChange(props.currentPage - 1)} variant="outline">
                  <ChevronLeft className="h-4 w-4" />
                </Button>
                <Button aria-label="Next page" className="h-8 w-8 px-0" disabled={props.currentPage >= props.totalPages} onClick={() => props.onPageChange(props.currentPage + 1)} variant="outline">
